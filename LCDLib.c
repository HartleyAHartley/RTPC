...
{
    UCB3CTLW0 = UCSWRST;
    UCB3CTLW0 |= UCCKPL | UCMSB | UCMST | UCMODE_0 | UCSSEL_2;
    UCB3CTLW0 &= ~UCSWRST;

    GPIO_setAsPeripheralModuleFunctionOutputPin(
            GPIO_PORT_P10,
            GPIO_PIN1 | // P10.1 - CLK
            GPIO_PIN2 | // P10.2 - MOSI
            GPIO_PIN3,  // P10.3 - MISO
            GPIO_PRIMARY_MODULE_FUNCTION
    );
    P10->DIR |= BIT4    // P10.4 - LCD CS
             |  BIT5;   // P10.5 - TP CS
}
...
void LCD_DrawRectangle(int16_t xStart, int16_t xEnd, int16_t yStart, int16_t yEnd, uint16_t Color)
{
    ...
    if(xEnd >= MAX_SCREEN_X)
        xEnd = MAX_SCREEN_X;
    if(xEnd < MIN_SCREEN_X)
        xEnd = MIN_SCREEN_X;
    if(yEnd >= MAX_SCREEN_Y)
        yEnd = MAX_SCREEN_Y;
    if(yEnd < MIN_SCREEN_Y)
        yEnd = MIN_SCREEN_Y;
...
uint16_t TP_ReadX(){
    ...
    val = (val-350)*MAX_SCREEN_X/(4095-384);
    return val;
}
...
uint16_t TP_ReadY(){
    ...
    val = SPISendRecvByte(0) << 5;
    val |= SPISendRecvByte(0) >> 3;
    val = (val-220)*MAX_SCREEN_Y/(4095-380);
    SPI_CS_TP_HIGH;
